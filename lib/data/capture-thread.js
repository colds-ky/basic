...
export function captureThread(threadView, store, now, intercepts) {
  ...
  return captureThreadViewPostOrVariants(visitedCIDs, threadView, undefined, store, now, intercepts);
}
 ...
 * @param {import('./define-store').Intercepts} [intercepts]
 */
function captureThreadViewPostOrVariants(visitedCIDs, threadViewPostOrVariants, parentPostHint, store, now, intercepts) {
  const threadViewPost = /** @type {import('@atproto/api').AppBskyFeedDefs.ThreadViewPost} */(
    threadViewPostOrVariants);
  
  if (threadViewPost?.post) return captureThreadViewPost(visitedCIDs, threadViewPost, store, now, intercepts);
 ...
 * @param {import('./define-store').Intercepts} [intercepts]
 */
function captureThreadViewPost(visitedCIDs, threadViewPost, store, now, intercepts) {
  const compactPost = capturePostView(visitedCIDs, threadViewPost.post, store, now, intercepts);
 ...
 * @param {import('./define-store').Intercepts} [intercepts]
 */
function capturePostView(visitedCIDs, postView, store, now, intercepts) {
