...
import { streamEvery } from '../../../src/api/akpa';
...
export async function* searchAccountHistoryPostsIncrementally(args) {
  ...
  const searchWordsPromises = words.map(word => 
    agent_searchPosts_throttled(word, undefined, 'latest')
  );
  ...
  const parallelSearch = streamEvery(
    /** @param {import('../../../src/api/akpa').StreamParameters<import('..').CompactPost[] | undefined>} streaming  */
    streaming => {
      const words = breakIntoWords(searchQuery || '');
      words.unshift(searchQuery || '');
