...
export function getModifiersTextSection(text, start, end) {
  ...
  let consequentMatch = /\S+\s*$/.exec(text.slice(0, start));
  const consequentEntryStart = start - (consequentMatch ? consequentMatch[0].length : 0);
  ...
  if (!consequentMatch || !consequentMatch[0]) {
    ...
    parsed = runParseRanges(consequentMatch[0], void 0);
    consequentEntry = parsed[0];
  } else {
    parsed = runParseRanges(consequentMatch[0], void 0);
    consequentEntry = parsed[parsed.length - 1];
  }
  ...
  if (typeof consequentEntry === 'string' && parsed && parsed.length > 1) {
    const prevConsequentEntry = parsed[parsed.length - 2];
